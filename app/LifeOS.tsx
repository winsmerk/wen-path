...
function Finance({ records, actions, busy, mutate }: { records: FinancialRecord[]; actions: Action[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  const cash = latest("cash"), investment = latest("investment"), income = flow("income"), expense = flow("fixed_expense");
  ...
  return <>
    ...
    <div className="finance-grid">
      ...
      <article><span>固定支出</span><b>{currency(expense)}</b><small>本月累计</small></article>
...
function FinanceDialog({ record, actions, busy, onClose, onSave, onDelete }: { record?: FinancialRecord; actions: Action[]; busy: boolean; onClose: () => void; onSave: (values: Record<string,unknown>) => void; onDelete?: () => void }) {
  ...
  const descriptionOnly = category === "fixed_asset" || category === "property";
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog" onSubmit={(event)=>{event.preventDefault();onSave({category,amount,recordedAt,note,actionId});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">财务记录</span><h2>{record ? "编辑财务记录" : "添加一条数据"}</h2><p>固定资产只记录事实描述，不估值，也不参与可计算资产合计。</p><div className={descriptionOnly ? "field-grid single" : "field-grid"}><label><span>类型</span><select value={category} onChange={(event)=>setCategory(event.target.value as FinancialRecord["category"])}><option value="cash">现金</option><option value="fixed_asset">固定资产</option><option value="investment">投资</option><option value="income">收入</option><option value="fixed_expense">固定支出</option></select></label>{!descriptionOnly&&<label><span>金额（元）</span><input required type="number" min="0" step="0.01" value={amount} onChange={(event)=>setAmount(event.target.value)} /></label>}</div><label><span>{descriptionOnly ? "固定资产描述" : "备注"}</span><textarea required={descriptionOnly} value={note} onChange={(event)=>setNote(event.target.value)} placeholder={descriptionOnly ? "例如：自住房一套，位于杭州，暂不估值" : "补充数据口径或说明（可选）"} /></label><label><span>关联任务（可选）</span><select value={actionId} onChange={(event)=>setActionId(event.target.value)}><option value="">不关联任务</option>{actions.map((item)=><option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label><span>日期</span><input type="date" value={recordedAt} onChange={(event)=>setDate(event.target.value)} /></label><div className="dialog-actions">{record&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} disabled={busy} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除记录"}</button>:<span/>}<button className="primary-button" disabled={busy}>{busy?"保存中…":"保存财务记录"}</button></div></form></div>;
}
