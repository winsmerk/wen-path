...
function Finance({ records, actions, busy, mutate }: { records: FinancialRecord[]; actions: Action[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  const cash = latest("cash"), reserveFund=latest("reserve_fund"), investment = latest("investment"), income = flow("income");
  ...
  const detailFilters:Record<string,{title:string;matches:(item:FinancialRecord)=>boolean}>={
    all:{title:"全部财务记录",matches:()=>true},cash:{title:"可用现金明细",matches:(item)=>item.category==="cash"},reserve:{title:"备用金明细",matches:(item)=>item.category==="reserve_fund"},assets:{title:"固定资产明细",matches:(item)=>item.category==="fixed_asset"||item.category==="property"},investment:{title:"投资明细",matches:(item)=>item.category==="investment"},income:{title:"本月收入明细",matches:(item)=>item.category==="income"&&item.recorded_at.startsWith(month)},incomeStructure:{title:"工资与非工资收入明细",matches:(item)=>item.category==="income"&&item.recorded_at.startsWith(month)},expense:{title:"本月支出明细",matches:(item)=>expenseTypes.includes(item.category)&&item.recorded_at.startsWith(month)},profit:{title:"经营利润明细",matches:(item)=>item.recorded_at.startsWith(month)&&((item.category==="income"&&item.income_type==="non_salary")||(expenseTypes.includes(item.category)&&item.expense_scope==="business"))},concentration:{title:"收入来源明细",matches:(item)=>item.category==="income"&&item.recorded_at.startsWith(month)},netWorth:{title:"可计算资产明细",matches:(item)=>["cash","reserve_fund","investment"].includes(item.category)},saving:{title:"本月结余明细",matches:(item)=>item.recorded_at.startsWith(month)&&(item.category==="income"||expenseTypes.includes(item.category))},
  };
  ...
  return <>
    ...
    <div className="finance-grid">
      ...
      {summary("investment","投资",currency(investment),"最新快照")}
      {summary("income","本月收入",currency(income),"累计记录")}
      {summary("incomeStructure","工资 / 非工资收入",`${currency(salaryIncome)} / ${currency(nonSalaryIncome)}`,"收入结构")}
      {summary("expense","本月总支出",currency(expense),"五类支出累计")}
      {summary("profit","经营利润",currency(nonSalaryIncome-businessExpense),"非工资收入 − 经营支出")}
      {summary("concentration","收入来源集中度",income?`${concentration}%`:"—","最大单一来源占比")}
      {summary("netWorth","可计算资产",currency(cash+reserveFund+investment),"现金 + 备用金 + 投资","net-worth")}
      {summary("saving","结余率",income ? `${Math.round((income - expense) / income * 100)}%` : "—","收入减总支出")}
    </div>
    <section className="finance-list"><div className="section-heading"><div><span className="eyebrow">{detail.title}</span><h3>{detailKey==="all"?"每笔数据都可编辑，也可追溯到任务":`共 ${visibleRecords.length} 条记录`}</h3></div>{detailKey!=="all"&&<button className="soft-button" onClick={()=>setDetailKey("all")}>查看全部</button>}</div>{visibleRecords.length ? visibleRecords.map((item) => {
