...
type JourneyTask = { id:string;journey_id:string;title:string;acceptance_criteria:string;estimated_minutes:number;task_type:"reading"|"finance"|"exercise"|"english"|"general";priority:number;status:"pending"|"completed";source:"manual"|"ai";completed_at:string|null };
...
function JourneyTaskDialog({journey,item,busy,onClose,onSave,onDelete}:{journey:Journey;item?:JourneyTask;busy:boolean;onClose:()=>void;onSave:(values:Record<string,unknown>)=>void;onDelete?:()=>void}){
  const [title,setTitle]=useState(item?.title||""),[acceptanceCriteria,setAcceptance]=useState(item?.acceptance_criteria||""),[estimatedMinutes,setMinutes]=useState(item?.estimated_minutes||60),[taskType,setTaskType]=useState<JourneyTask["task_type"]>(item?.task_type||"general"),[confirmDelete,setConfirmDelete]=useState(false);
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog task-dialog" onSubmit={(event)=>{event.preventDefault();onSave({title,acceptanceCriteria,estimatedMinutes,taskType});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">征程子任务</span><h2>{item?"编辑子任务":"创建子任务"}</h2><p>归属「{journey.title}」。月计划和周计划会从这些子任务中选取。</p><label><span>任务名称</span><input required maxLength={120} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="一个明确、可独立完成的行动" /></label><label><span>完成标准</span><textarea required maxLength={500} value={acceptanceCriteria} onChange={(event)=>setAcceptance(event.target.value)} placeholder="留下什么结果，才算完成？" /></label><div className="field-grid"><label><span>预计分钟</span><input type="number" min="15" max="1200" step="15" value={estimatedMinutes} onChange={(event)=>setMinutes(Number(event.target.value))} /></label><label><span>任务类型</span><select value={taskType} onChange={(event)=>setTaskType(event.target.value as JourneyTask["task_type"])}><option value="general">通用</option><option value="reading">阅读</option><option value="finance">财务</option><option value="exercise">运动</option><option value="english">英语</option></select></label></div><div className="dialog-actions">{item&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除子任务"}</button>:<span/>}<button className="primary-button" disabled={busy||!title.trim()||!acceptanceCriteria.trim()}>{busy?"保存中…":"保存子任务"}</button></div></form></div>;
}
