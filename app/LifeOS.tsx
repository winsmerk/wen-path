...
function OutcomeCard({ item, index, onEdit }: { item: Outcome; index: number; onEdit?: (item: Outcome) => void }) {
  ...
  return <article className="outcome-card"><div className="outcome-head"><span className={`area-pill ${tone}`}>{label}</span><div><b>{item.progress}%</b>{onEdit && <button type="button" className="card-edit" onClick={() => onEdit(item)}>编辑</button>}</div></div><h4>{item.title}</h4><p>{item.acceptance_criteria}</p><div className="progress-bar"><i className={tone} style={{ width: `${item.progress}%` }} /></div></article>;
}
...
function Finance({ records, actions, busy, mutate }: { records: FinancialRecord[]; actions: Action[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  return <>
    ...
    <section className="finance-list"><div className="section-heading"><div><span className="eyebrow">全部财务记录</span><h3>每笔数据都可编辑，也可追溯到任务</h3></div></div>{records.length ? records.map((item) => {
      ...
      return <article key={item.id}><span className={`finance-category ${item.category}`}>{financeLabel(item.category)}</span><div><b>{descriptionOnly ? item.note || "固定资产" : currency(item.amount)}</b><p>{descriptionOnly ? "不估值、不计入可计算资产" : item.note || "未填写备注"}{item.action_id ? ` · 关联：${actions.find((action) => action.id === item.action_id)?.title || "任务"}` : ""}</p></div><div className="finance-record-meta"><time>{new Date(item.recorded_at).toLocaleDateString("zh-CN")}</time><button onClick={() => setEditing(item)}>编辑</button></div></article>;
...
function OutcomeDialog({ item, busy, onClose, onSave, onDelete }: { item?: Outcome; busy: boolean; onClose: () => void; onSave: (values: Record<string, unknown>) => void; onDelete?: () => void }) {
  const [title,setTitle]=useState(item?.title ?? ""),[acceptanceCriteria,setAcceptance]=useState(item?.acceptance_criteria ?? ""),[progress,setProgress]=useState(item?.progress ?? 0),[expectedHours,setExpectedHours]=useState(item?.expected_hours ?? 6),[confirmDelete,setConfirmDelete]=useState(false);
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog outcome-dialog" onSubmit={(event)=>{event.preventDefault();onSave({title,acceptanceCriteria,progress,expectedHours});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">月度成果</span><h2>{item ? "编辑月度成果" : "新增月度成果"}</h2><p>成果描述结果，完成标准负责判断是否真正完成。</p><label><span>成果名称</span><input required maxLength={100} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="例如：完成个人作品集第一版" /></label><label><span>完成标准</span><textarea required maxLength={500} value={acceptanceCriteria} onChange={(event)=>setAcceptance(event.target.value)} placeholder="达到什么状态才算完成？" /></label><div className="field-grid"><label><span>预计投入（小时）</span><input required type="number" min="1" max="200" value={expectedHours} onChange={(event)=>setExpectedHours(Number(event.target.value))} /></label><label><span>当前进度（%）</span><input required type="number" min="0" max="100" value={progress} onChange={(event)=>setProgress(Number(event.target.value))} /></label></div><div className="dialog-actions">{item&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} disabled={busy} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除成果"}</button>:<span/>}<button className="primary-button" disabled={busy}>{busy?"保存中…":"保存月度成果"}</button></div></form></div>;
}

function WeeklyActionDialog({ item, outcomes, busy, onClose, onSave }: { item?: Action; outcomes: Outcome[]; busy: boolean; onClose: () => void; onSave: (values: Record<string, unknown>) => void }) {
  const [title,setTitle]=useState(item?.title ?? ""),[outcomeId,setOutcomeId]=useState(item?.outcome_id ?? outcomes[0]?.id ?? ""),[estimatedMinutes,setMinutes]=useState(item?.estimated_minutes ?? 30),[scheduledFor,setDay]=useState(item?.scheduled_for ?? "本周"),[taskType,setTaskType]=useState<Action["task_type"]>(item?.task_type ?? "general");
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog task-dialog" onSubmit={(event)=>{event.preventDefault();onSave({title,outcomeId,estimatedMinutes,scheduledFor,taskType});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">本周任务</span><h2>{item ? "编辑周任务" : "新增周任务"}</h2><p>关联月度成果后，每周行动会更容易保持方向。</p><label><span>任务名称</span><input required maxLength={120} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="写下一个明确可完成的行动" /></label><label><span>关联月度成果（可选）</span><select value={outcomeId} onChange={(event)=>setOutcomeId(event.target.value)}><option value="">暂不关联</option>{outcomes.map((outcome)=><option key={outcome.id} value={outcome.id}>{outcome.title}</option>)}</select></label><div className="field-grid"><label><span>安排时间</span><select value={scheduledFor} onChange={(event)=>setDay(event.target.value)}>{["本周","周一","周二","周三","周四","周五","周六","周日"].map((day)=><option key={day}>{day}</option>)}</select></label><label><span>预计分钟</span><input required type="number" min="15" max="600" step="5" value={estimatedMinutes} onChange={(event)=>setMinutes(Number(event.target.value))} /></label></div><label><span>任务类型</span><select value={taskType} onChange={(event)=>setTaskType(event.target.value as Action["task_type"])}><option value="general">通用</option><option value="reading">阅读</option><option value="finance">财务</option><option value="exercise">运动</option><option value="english">英语</option></select></label><button className="primary-button full" disabled={busy}>{busy?"保存中…":item?"保存任务修改":"添加到本周"}</button></form></div>;
}
...
function FinanceDialog({ record, actions, busy, onClose, onSave, onDelete }: { record?: FinancialRecord; actions: Action[]; busy: boolean; onClose: () => void; onSave: (values: Record<string,unknown>) => void; onDelete?: () => void }) {
  ...
  const [category,setCategory]=useState<FinancialRecord["category"]>(initialCategory),[amount,setAmount]=useState(record && initialCategory !== "fixed_asset" ? String(record.amount) : ""),[note,setNote]=useState(record?.note ?? ""),[recordedAt,setDate]=useState(record?.recorded_at ?? new Date().toISOString().slice(0,10)),[actionId,setActionId]=useState(record?.action_id ?? ""),[confirmDelete,setConfirmDelete]=useState(false);
  const descriptionOnly = category === "fixed_asset" || category === "property";
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog" onSubmit={(event)=>{event.preventDefault();onSave({category,amount,recordedAt,note,actionId});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">财务记录</span><h2>{record ? "编辑财务记录" : "添加一条数据"}</h2><p>固定资产只记录事实描述；五类支出会统一计入本月总支出。</p><div className={descriptionOnly ? "field-grid single" : "field-grid"}><label><span>类型</span><select value={category} onChange={(event)=>setCategory(event.target.value as FinancialRecord["category"])}><option value="cash">现金</option><option value="fixed_asset">固定资产</option><option value="investment">投资</option><option value="income">收入</option><option value="fixed_expense">固定支出</option><option value="daily_expense">日常消费</option><option value="social_expense">请客</option><option value="exercise_expense">运动</option><option value="learning_expense">学习</option></select></label>{!descriptionOnly&&<label><span>金额（元）</span><input required type="number" min="0" step="0.01" value={amount} onChange={(event)=>setAmount(event.target.value)} /></label>}</div><label><span>{descriptionOnly ? "固定资产描述" : "备注"}</span><textarea required={descriptionOnly} value={note} onChange={(event)=>setNote(event.target.value)} placeholder={descriptionOnly ? "例如：自住房一套，位于杭州，暂不估值" : "补充数据口径或说明（可选）"} /></label><label><span>关联任务（可选）</span><select value={actionId} onChange={(event)=>setActionId(event.target.value)}><option value="">不关联任务</option>{actions.map((item)=><option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label><span>日期</span><input type="date" value={recordedAt} onChange={(event)=>setDate(event.target.value)} /></label><div className="dialog-actions">{record&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} disabled={busy} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除记录"}</button>:<span/>}<button className="primary-button" disabled={busy}>{busy?"保存中…":"保存财务记录"}</button></div></form></div>;
}
