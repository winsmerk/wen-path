...
type JourneyTask = { id:string;journey_id:string;title:string;main_task:string;acceptance_criteria:string;estimated_minutes:number;task_type:"reading"|"finance"|"exercise"|"english"|"account_operation"|"general";execution_frequency:"once"|"daily"|"weekly"|"monthly";preferred_weekday:string;preferred_month_day:number;preferred_time:string;priority:number;status:"pending"|"completed";source:"manual"|"ai";completed_at:string|null };
...
type Action = {
  ...
  task_type: "reading" | "finance" | "exercise" | "english" | "account_operation" | "general";
...
type FinancialRecord = { id: string; action_id: string | null; category: "cash" | "reserve_fund" | "fixed_asset" | "investment" | "property" | "income" | "fixed_expense" | "daily_expense" | "social_expense" | "exercise_expense" | "learning_expense" | "travel_expense"; amount: number; note: string; recorded_at: string; income_type: "" | "salary" | "non_salary"; source_name: string; expense_scope: "personal" | "business";investment_principal:number;investment_return:number };
type FinancialMonthlyBill={id:string;period:string;income_total:number;salary_income:number;non_salary_income:number;expense_total:number;business_expense:number;investment_principal:number;investment_return:number;business_profit:number;net_cash_flow:number;settled_at:string};
...
type RecordImage = { id: string; record_type: "checkin" | "task_output" | "planning_record"; record_id: string };
type Memo = { id:string;title:string;content:string;remind_at:string;status:"pending"|"completed";wechat_enabled:number;delivery_status:"not_requested"|"pending"|"sending"|"sent"|"failed";attempt_count:number;sent_at:string|null;last_error:string;created_at:string;updated_at:string };
...
type Workspace = {
  ...
  financialMonthlyBills:FinancialMonthlyBill[];
...
export default function LifeOS() {
  ...
  async function mutate(payload: Record<string, unknown>, success?: string) {
    ...
    if (!response.ok) {
      ...
      const messages:Record<string,string>={stage_locked:"前一阶段尚未通关，暂时不能激活这项征程。",evidence_required:"请先提交完成证据。",evidence_needs_more:`验收未通过（${result.score??0}分）：${result.feedback||"请补充可验证证据。"}`,month_outcomes_full:"本月已经有5项成果，请先完成或调整现有成果。",month_generated_full:"本月已从子任务生成5项成果，可编辑或删除后重新规划。",month_not_ready:"每月25日后开放月末结算，避免过早放弃本月成果。",month_settled:"本月已经结算，请到下月继续规划。",already_carried:"这项任务已经结转到本周。",no_journey_tasks:"当前进行中的征程没有未完成子任务，请先创建子任务或激活要推进的征程。",no_tasks_fit_capacity:"没有子任务能放入当前时间容量，请调整可用时间或拆小任务。",generation_failed:"AI 暂时没有生成有效子任务，请稍后重试。",invalid_source_task:"请选择一个未完成的征程子任务。"};
...
function Today(props: {
  ...
  const today = new Date();
  const quote = dailySuShiQuote(today.toISOString().slice(0,10));
  ...
  return (
    <>
      ...
      <section className="hero-grid">
        <article className="focus-card">
          ...
          {pending ? <>
            ...
            <div className="action-row"><button className="primary-button" disabled={props.busy} onClick={() => props.onToggle(pending)}>✓ 完成行动</button><button className="soft-button" onClick={() => props.onNavigate("records")}>查看记录</button></div>
      ...
      <section className="dashboard-section">
        <div className="section-heading"><div><span className="eyebrow">行动记录</span><h3>本月留下的成果</h3></div><button className="text-button" onClick={() => props.onNavigate("records")}>查看全部记录 →</button></div>
...
function Plan({ profile, journeys, journeyTasks, outcomes, outcomeHistory, activeWeek, weeklyCycles, actions, historyActions, reviews, busy, mutate, onComplete }: { profile: Profile; journeys: Journey[];journeyTasks:JourneyTask[]; outcomes: Outcome[]; outcomeHistory:Outcome[];activeWeek:WeeklyCycle;weeklyCycles:WeeklyCycle[];actions: Action[];historyActions:Action[]; reviews: Review[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean>; onComplete: (action: Action) => void }) {
  ...
  const [selectedTaskIds,setSelectedTaskIds]=useState<string[]>(()=>outcomes.map((item)=>item.source_task_id).filter(Boolean));
  ...
  return <>
    ...
    {!showHistory&&<>
    <section className="week-goal-card"><div><span className="eyebrow">本周目标</span><h3>{profile.weekly_goal || "还没有填写本周目标"}</h3><p>周计划只从本月已选主任务的周期子任务中生成；没有日期配置的任务保留为“本周择时”。</p></div><div><b>{Math.round(profile.weekly_capacity_minutes / 6) / 10}h</b><small>本周可用</small></div><button className="primary-button" disabled={busy} onClick={() => mutate({ action: "generate-week-plan" }, "已按任务规则生成本周计划")}>{busy ? "排期中…" : "按规则生成周计划"}</button></section>
    ...
    <section className="plan-layout">
      ...
      <div><div className="section-heading"><div><span className="eyebrow">月度计划 · {outcomes[0]?.period||new Date().toISOString().slice(0,7)}</span><h3>勾选本月要推进的主任务（最多8项）</h3></div><div className="monthly-actions"><button className="soft-button" disabled={busy||!selectedTaskIds.length} onClick={()=>mutate({action:"generate-month-outcomes",taskIds:selectedTaskIds},"已按所选主任务生成月计划")}>{busy?"规划中…":"生成月计划"}</button><button className={confirmSettlement?"danger-button confirm":"soft-button"} disabled={busy} onClick={()=>confirmSettlement?mutate({action:"settle-month"},"本月已结算，未完成成果已滚动到下月").then((ok)=>{if(ok)setConfirmSettlement(false);}):setConfirmSettlement(true)}>{confirmSettlement?"确认结算并滚动":"月末结算"}</button></div></div><div className="monthly-task-picker">{journeyTasks.filter((task)=>task.status==="pending"&&journeys.some((level)=>level.id===task.journey_id&&level.status==="active")).map((task)=><label key={task.id} className={selectedTaskIds.includes(task.id)?"selected":""}><input type="checkbox" checked={selectedTaskIds.includes(task.id)} disabled={!selectedTaskIds.includes(task.id)&&selectedTaskIds.length>=8} onChange={()=>setSelectedTaskIds((current)=>current.includes(task.id)?current.filter((id)=>id!==task.id):[...current,task.id])}/><span><b>{task.main_task||task.title}</b><small>{task.title} · {{once:"单次",daily:"每日",weekly:"每周",monthly:"每月"}[task.execution_frequency]}</small></span></label>)}</div><div className="plan-outcomes">{outcomes.map((item, index) => <OutcomeCard item={item} index={index} key={item.id} onEdit={setEditingOutcome} />)}{outcomes.length === 0 && <div className="empty-list"><b>还没有月度计划</b><p>勾选当前关卡中的主任务并生成。</p></div>}</div></div>
...
function Records({ items, outputs, planningRecords, taskTypes, journals, journalImages, recordImages, onReload }: { items: Checkin[]; outputs: TaskOutput[]; planningRecords: PlanningRecord[]; taskTypes: TaskType[]; journals: JournalEntry[]; journalImages: JournalImage[]; recordImages: RecordImage[]; onReload: () => Promise<void> }) {
  ...
  const typeCount = (key:string) => planningRecords.filter((item)=>item.type_key===key).length + items.filter((item)=>legacyRecordType(item.type)===key).length + outputs.filter((item)=>legacyRecordType(item.task_type)===key).length;
  return <>
    <PageHeader kicker="行动留下痕迹" title="记录"><div className="action-row"><button className="soft-button" onClick={() => setEditingJournal("diary")}>＋ 写日记</button><button className="primary-button" onClick={() => setEditingJournal("inspiration")}>＋ 记灵感</button></div></PageHeader>
...
function MemoPanel() {
  const [memos,setMemos]=useState<Memo[]>([]),[wechatConfigured,setWechatConfigured]=useState(false),[editing,setEditing]=useState<Memo|"new"|null>(null),[newMemoTime,setNewMemoTime]=useState(""),[filter,setFilter]=useState<"pending"|"completed"|"all">("pending"),[busy,setBusy]=useState(false),[message,setMessage]=useState(""),[clock,setClock]=useState(() => new Date());
  const load=useCallback(async()=>{const response=await fetch("/api/memos",{cache:"no-store"});if(!response.ok)return;const data=await response.json() as {memos:Memo[];wechatConfigured:boolean};setMemos(data.memos);setWechatConfigured(data.wechatConfigured);},[]);
...
function MemoDialog({memo,defaultRemindAt,busy,onClose,onSave,onDelete}:{memo?:Memo;defaultRemindAt:string;busy:boolean;onClose:()=>void;onSave:(values:Record<string,unknown>)=>void;onDelete?:()=>void}){
  const [title,setTitle]=useState(memo?.title??""),[content,setContent]=useState(memo?.content??""),[remindAt,setRemindAt]=useState(memo?localDateTimeInput(new Date(memo.remind_at)):defaultRemindAt),[wechatEnabled,setWechatEnabled]=useState(memo?Boolean(memo.wechat_enabled):true),[confirmDelete,setConfirmDelete]=useState(false);
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog memo-dialog" onSubmit={(event)=>{event.preventDefault();onSave({title,content,remindAt:new Date(remindAt).toISOString(),wechatEnabled});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">备忘录</span><h2>{memo?"编辑备忘":"添加一件要记住的事"}</h2><p>设置提醒时间；开启微信提醒后，到时会自动发送消息。</p><label><span>标题</span><input required maxLength={120} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="例如：续费护照" /></label><label><span>详细内容（可选）</span><textarea maxLength={4000} value={content} onChange={(event)=>setContent(event.target.value)} placeholder="补充地址、材料或要做的步骤……" /></label><label><span>提醒时间</span><input required type="datetime-local" value={remindAt} onChange={(event)=>setRemindAt(event.target.value)} /></label><label className="check-field"><input type="checkbox" checked={wechatEnabled} onChange={(event)=>setWechatEnabled(event.target.checked)} /><span>到时发送微信提醒</span></label><div className="dialog-actions">{onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} disabled={busy} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除备忘"}</button>:<span/>}<button className="primary-button" disabled={busy||!title.trim()}>{busy?"保存中…":"保存备忘"}</button></div></form></div>;
}
...
function Finance({ records,bills, busy, mutate }: { records: FinancialRecord[];bills:FinancialMonthlyBill[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  const monthRecords=records.filter((item)=>item.recorded_at.startsWith(selectedPeriod));
  const flow = (category: FinancialRecord["category"]) => monthRecords.filter((item) => item.category === category).reduce((sum, item) => sum + item.amount, 0);
  const cash = latest("cash"), reserveFund=latest("reserve_fund"), income = flow("income");
  const expenseTypes: FinancialRecord["category"][] = ["fixed_expense", "daily_expense", "social_expense", "exercise_expense", "learning_expense", "travel_expense"];
  const expense = expenseTypes.reduce((sum, category) => sum + flow(category), 0);
  const monthlyIncomeRecords = monthRecords.filter((item)=>item.category==="income");
  ...
  const businessExpense = monthRecords.filter((item)=>expenseTypes.includes(item.category)&&item.expense_scope==="business").reduce((sum,item)=>sum+item.amount,0);
  const investmentRecords=monthRecords.filter((item)=>item.category==="investment"),investmentPrincipal=investmentRecords.reduce((sum,item)=>sum+Number(item.investment_principal||item.amount),0),investmentReturn=investmentRecords.reduce((sum,item)=>sum+Number(item.investment_return||0),0);
  ...
  const displayCurrency = (value: number) => showAmounts ? currency(value) : "¥••••";
  const bill=bills.find((item)=>item.period===selectedPeriod),isSettled=selectedPeriod<currentPeriod&&Boolean(bill);
  const totals=isSettled&&bill?{income:bill.income_total,salary:bill.salary_income,nonSalary:bill.non_salary_income,expense:bill.expense_total,principal:bill.investment_principal,investmentReturn:bill.investment_return,profit:bill.business_profit,netFlow:bill.net_cash_flow}:{income,salary:salaryIncome,nonSalary:nonSalaryIncome,expense,principal:investmentPrincipal,investmentReturn,profit:nonSalaryIncome-businessExpense,netFlow:income+investmentReturn-expense};
  ...
  const visibleAmount=visibleRecords.reduce((sum,item)=>sum+Number(item.amount||0),0),visibleInvestmentPrincipal=visibleRecords.reduce((sum,item)=>sum+(item.category==="investment"?Number(item.investment_principal||item.amount):0),0),visibleInvestmentReturn=visibleRecords.reduce((sum,item)=>sum+(item.category==="investment"?Number(item.investment_return||0):0),0),descriptionOnlyTotal=visibleRecords.length>0&&visibleRecords.every((item)=>item.category==="fixed_asset"||item.category==="property");
  ...
  return <>
    <PageHeader kicker="固定资产与月度经营账" title="当前财务情况"><div className="action-row finance-header-actions"><button type="button" className="soft-button amount-visibility-button" aria-pressed={showAmounts} onClick={() => setShowAmounts((visible) => !visible)}>{showAmounts ? "◉ 隐藏金额" : "◎ 显示金额"}</button><button className="primary-button" onClick={() => setAdding(true)}>＋ 添加财务记录</button></div></PageHeader>
    <section className="finance-section"><div className="section-heading"><div><span className="eyebrow">固定资产模块</span><h3>现金与资产快照</h3></div></div><div className="finance-grid fixed-assets-grid">
      {summary("cash","可用现金",displayCurrency(cash),"最新快照")}
      {summary("reserve","备用金",displayCurrency(reserveFund),"最新快照")}
      {summary("assets","固定资产",`${fixedAssetCount} 项`,"只记录描述，不估值")}
      {summary("fixedTotal","可计算固定资产",displayCurrency(cash+reserveFund),"现金 + 备用金","net-worth")}
    </div></section>
    <section className="finance-section monthly-bill"><div className="section-heading"><div><span className="eyebrow">月度账单</span><h3>{selectedPeriod} · {isSettled?"已自动结算":"本月实时统计"}</h3></div><select value={selectedPeriod} onChange={(event)=>{setSelectedPeriod(event.target.value);chooseDetail("all");}}>{periods.map((period)=><option key={period} value={period}>{period}{period<currentPeriod?" · 已结算":" · 本月"}</option>)}</select></div><div className="bill-status"><span className={isSettled?"settled":"live"}>{isSettled?"✓ 月底已结算":"● 实时账单"}</span><p>{isSettled?`结算时间 ${new Date(bill!.settled_at).toLocaleDateString("zh-CN")}`:"进入下个月后，系统会自动锁定本月汇总并生成历史账单。"}</p></div><div className="finance-grid">
      {summary("investment","投资本金 / 月收益",`${displayCurrency(totals.principal)} / ${displayCurrency(totals.investmentReturn)}`,totals.investmentReturn<0?"本月投资亏损":"本月投资收益")}
      {summary("income","本月收入",displayCurrency(totals.income),"累计记录")}
      {summary("incomeStructure","工资 / 非工资收入",`${displayCurrency(totals.salary)} / ${displayCurrency(totals.nonSalary)}`,"收入结构")}
      {summary("expense","本月总支出",displayCurrency(totals.expense),"六类支出累计")}
      {summary("profit","经营利润",displayCurrency(totals.profit),"非工资收入 − 经营支出")}
      {summary("netFlow","月度净流入",displayCurrency(totals.netFlow),"收入 + 投资收益 − 支出","net-worth")}
    </div></section>
    <section className="finance-list"><div className="section-heading"><div><span className="eyebrow">{detail.title}</span><h3>{detailKey==="all"?`${isSettled?"已结算账单":"实时账单"} · ${visibleRecords.length} 条记录`:`共 ${visibleRecords.length} 条记录`}</h3></div>{detailKey!=="all"&&<button className="soft-button" onClick={()=>chooseDetail("all")}>查看月账单全部</button>}</div>{categoryOptions.length>1&&<div className="finance-detail-filters" aria-label="明细分类筛选">{categoryOptions.map((option)=><button type="button" key={option.key} className={detailCategory===option.key?"active":""} onClick={()=>setDetailCategory(option.key)}>{option.label}</button>)}</div>}<div className="finance-filter-total"><div><span>{categoryFilter?.label||detail.title}</span><small>当前筛选 · {visibleRecords.length} 条记录</small></div>{descriptionOnlyTotal?<strong>{visibleRecords.length} 项<span>固定资产不估值</span></strong>:detailKey==="investment"?<div className="investment-filter-totals"><strong>{displayCurrency(visibleInvestmentPrincipal)}<span>本金合计</span></strong><strong className={visibleInvestmentReturn<0?"negative":""}>{displayCurrency(visibleInvestmentReturn)}<span>收益合计</span></strong></div>:<strong>{displayCurrency(visibleAmount)}<span>金额合计</span></strong>}</div>{visibleRecords.length ? visibleRecords.map((item) => {
      const descriptionOnly = item.category === "fixed_asset" || item.category === "property";
      return <article key={item.id}><span className={`finance-category ${item.category}`}>{financeLabel(item.category)}</span><div><b>{descriptionOnly ? item.note || "固定资产" : item.category==="investment"?`本金 ${displayCurrency(item.investment_principal||item.amount)} · 收益 ${displayCurrency(item.investment_return||0)}`:displayCurrency(item.amount)}</b><p>{descriptionOnly ? "不估值、不计入可计算资产" : `${item.note || "未填写备注"}${item.category==="income"?` · ${item.income_type==="salary"?"工资":"非工资"}${item.source_name?` / ${item.source_name}`:""}`:/expense$/.test(item.category)?` · ${item.expense_scope==="business"?"经营支出":"个人支出"}`:""}`}</p></div><div className="finance-record-meta"><time>{new Date(item.recorded_at).toLocaleDateString("zh-CN")}</time><button onClick={() => setEditing(item)}>编辑</button></div></article>;
    }) : <div className="empty-list"><b>这里还没有记录</b><p>点击“添加财务记录”，补充这部分财务数据。</p></div>}</section>
    {adding && <FinanceDialog busy={busy} onClose={() => setAdding(false)} onSave={async (values) => { const ok = await mutate({ action: "financial-record", ...values }, "财务记录已保存"); if (ok) setAdding(false); }} />}
    {editing && <FinanceDialog record={editing} busy={busy} onClose={() => setEditing(null)} onSave={async (values) => { const ok = await mutate({ action: "update-financial-record", id: editing.id, ...values }, "财务记录已更新"); if (ok) setEditing(null); }} onDelete={async () => { const ok = await mutate({ action: "delete-financial-record", id: editing.id }, "财务记录已删除"); if (ok) setEditing(null); }} />}
  </>;
}

function financeLabel(category: FinancialRecord["category"]) { return { cash: "现金", reserve_fund:"备用金", fixed_asset: "固定资产", investment: "投资", property: "固定资产", income: "收入", fixed_expense: "固定支出", daily_expense: "日常消费", social_expense: "请客", exercise_expense: "运动", learning_expense: "学习", travel_expense: "旅行" }[category]; }
...
function ReviewPanel({ completedActions, actionTotal, exerciseCount, englishCount, reviews, reports, taskTypes, busy, mutate }: { completedActions: number; actionTotal: number; exerciseCount: number; englishCount: number; reviews: Review[]; reports:PlanningReport[];taskTypes:TaskType[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  const [selectedWeek,setSelectedWeek]=useState<{report:PlanningReport;review?:Review}|null>(null);
  const reportSummary=(report:PlanningReport)=>{try{return JSON.parse(report.summary_json) as {total:number;completed:number;pending:number;paused:number;completionRate:number;plannedMinutes:number;completedMinutes:number;byType:Record<string,{count:number;minutes:number;completed:number}>};}catch{return {total:0,completed:0,pending:0,paused:0,completionRate:0,plannedMinutes:0,completedMinutes:0,byType:{}};}};
  const weekEnd=(start:string)=>{const end=new Date(`${start}T00:00:00Z`);end.setUTCDate(end.getUTCDate()+6);return end.toISOString().slice(0,10);};
  const periodLabel=(start:string)=>`${start.replace(/-/g,".")} — ${weekEnd(start).replace(/-/g,".")}`;
  async function submit(event: FormEvent) {
  ...
  return <>
    ...
    <section className="review-history"><div className="section-heading"><div><span className="eyebrow">持续积累</span><h3>历史复盘记录</h3></div><div className="history-filters"><button className={historyFilter==="all"?"active":""} onClick={()=>setHistoryFilter("all")}>全部</button><button className={historyFilter==="manual"?"active":""} onClick={()=>setHistoryFilter("manual")}>手动复盘</button><button className={historyFilter==="auto"?"active":""} onClick={()=>setHistoryFilter("auto")}>自动周报</button></div></div><div className="review-history-list">
      {weeklyReports.filter((report)=>historyFilter!=="manual"||reviews.some((review)=>review.week_start===report.period)).map((report)=>{const summary=reportSummary(report),manual=reviews.find((review)=>review.week_start===report.period);return <article className="history-review auto-report merged-report" key={`report-${report.id}`}><div className="history-review-head"><span className="history-kind auto">综合周报</span>{manual&&<span className="history-kind manual">含手动复盘</span>}<time>{periodLabel(report.period)}</time><b>{summary.completionRate}% 完成</b></div><h4>{manual?.achievement||`完成 ${summary.completed}/${summary.total} 项任务`}</h4><p>自动统计：完成 {summary.completed} 项、待完成 {summary.pending} 项，实际投入 {Math.round(summary.completedMinutes/60*10)/10} 小时。{manual?` 手动总结：下周重点是“${manual.next_priority}”。`:" 本周尚未填写手动复盘。"}</p><div className="history-type-tags">{Object.entries(summary.byType||{}).map(([key,value])=><span key={key}>{taskTypes.find((item)=>item.type_key===key)?.name||key} {value.completed}/{value.count}</span>)}</div><button className="report-detail-button" onClick={()=>setSelectedWeek({report,review:manual})}>查看周报详情 <span>→</span></button></article>})}
      {(historyFilter!=="auto")&&reviews.filter((review)=>!weeklyReports.some((report)=>report.period===review.week_start)).map((review)=><article className="history-review manual-review" key={`review-${review.id}`}><div className="history-review-head"><span className="history-kind manual">手动复盘 · 待周报汇合</span><time>{review.week_start?periodLabel(review.week_start):new Date(review.created_at).toLocaleDateString("zh-CN")}</time><b>{{continue:"继续",adjust:"调整",stop:"停止"}[review.decision]}</b></div><h4>{review.achievement||"本周复盘"}</h4><p>下周重点：{review.next_priority} · 能量 {review.energy_score}/10</p>{review.low_value&&<small>低价值消耗：{review.low_value}</small>}</article>)}
      {!weeklyReports.length&&!reviews.length&&<div className="empty-list"><b>还没有历史复盘</b><p>保存本周复盘后会记录在这里；自然周结束后系统也会自动生成周报。</p></div>}
    </div></section>
    {selectedWeek&&<WeeklyReportDialog report={selectedWeek.report} review={selectedWeek.review} taskTypes={taskTypes} onClose={()=>setSelectedWeek(null)}/>}
  </>;
}

function WeeklyReportDialog({report,review,taskTypes,onClose}:{report:PlanningReport;review?:Review;taskTypes:TaskType[];onClose:()=>void}){
  let summary:{total:number;completed:number;pending:number;paused:number;completionRate:number;plannedMinutes:number;completedMinutes:number;byType:Record<string,{count:number;minutes:number;completed:number}>};try{summary=JSON.parse(report.summary_json);}catch{summary={total:0,completed:0,pending:0,paused:0,completionRate:0,plannedMinutes:0,completedMinutes:0,byType:{}};}
  const end=new Date(`${report.period}T00:00:00Z`);end.setUTCDate(end.getUTCDate()+6);const endDate=end.toISOString().slice(0,10),hours=(minutes:number)=>`${Math.round(minutes/60*10)/10} 小时`;
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><div className="dialog weekly-report-dialog"><button className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">综合周报</span><h2>{report.period.replace(/-/g,".")} — {endDate.replace(/-/g,".")}</h2><div className="report-period-grid"><span><small>开始时间</small><b>{report.period}</b></span><span><small>结束时间</small><b>{endDate}</b></span><span><small>任务完成率</small><b>{summary.completionRate}%</b></span></div><section className="report-detail-section"><h3>任务执行情况</h3><div className="report-detail-metrics"><span><b>{summary.completed}/{summary.total}</b>完成任务</span><span><b>{summary.pending}</b>待完成</span><span><b>{hours(summary.completedMinutes)}</b>实际投入</span><span><b>{hours(summary.plannedMinutes)}</b>计划投入</span></div><div className="history-type-tags">{Object.entries(summary.byType||{}).map(([key,value])=><span key={key}>{taskTypes.find((item)=>item.type_key===key)?.name||key}：{value.completed}/{value.count} · {hours(value.minutes)}</span>)}</div></section>{review?<section className="report-detail-section manual-summary"><h3>手动复盘总结</h3><dl><div><dt>本周成果</dt><dd>{review.achievement}</dd></div><div><dt>低价值消耗</dt><dd>{review.low_value||"未填写"}</dd></div><div><dt>健康状态</dt><dd>{review.health_check}</dd></div><div><dt>市场证据</dt><dd>{review.market_evidence}</dd></div><div><dt>下周重点</dt><dd>{review.next_priority}</dd></div><div><dt>能量与决策</dt><dd>{review.energy_score}/10 · {{continue:"继续",adjust:"调整",stop:"停止"}[review.decision]}</dd></div></dl></section>:<section className="report-detail-section no-manual"><h3>手动复盘</h3><p>这一周没有手动复盘记录，当前周报仅包含系统自动统计。</p></section>}<button className="primary-button full" onClick={onClose}>关闭详情</button></div></div>;
}
...
function JourneyTaskDialog({journey,item,busy,onClose,onSave,onDelete}:{journey:Journey;item?:JourneyTask;busy:boolean;onClose:()=>void;onSave:(values:Record<string,unknown>)=>void;onDelete?:()=>void}){
  const [mainTask,setMainTask]=useState(item?.main_task||""),[title,setTitle]=useState(item?.title||""),[acceptanceCriteria,setAcceptance]=useState(item?.acceptance_criteria||""),[estimatedMinutes,setMinutes]=useState(item?.estimated_minutes||60),[taskType,setTaskType]=useState<JourneyTask["task_type"]>(item?.task_type||"general"),[executionFrequency,setExecutionFrequency]=useState<JourneyTask["execution_frequency"]>(item?.execution_frequency||"weekly"),[preferredWeekday,setPreferredWeekday]=useState(item?.preferred_weekday||""),[preferredMonthDay,setPreferredMonthDay]=useState(item?.preferred_month_day||0),[preferredTime,setPreferredTime]=useState(item?.preferred_time||""),[confirmDelete,setConfirmDelete]=useState(false);
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog task-dialog" onSubmit={(event)=>{event.preventDefault();onSave({mainTask,title,acceptanceCriteria,estimatedMinutes,taskType,executionFrequency,preferredWeekday:executionFrequency==="weekly"?preferredWeekday:"",preferredMonthDay:executionFrequency==="monthly"?preferredMonthDay:0,preferredTime});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">关卡任务设计</span><h2>{item?"编辑周期子任务":"创建主任务与子任务"}</h2><p>归属关卡「{journey.title}」。未配置日期或时间时，计划中保留“择时”，不会随机指定。</p><label><span>主任务</span><input required maxLength={120} value={mainTask} onChange={(event)=>setMainTask(event.target.value)} placeholder="例如：建立稳定的英语学习体系" /></label><label><span>可执行子任务</span><input required maxLength={120} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="例如：完成一节听力练习" /></label><label><span>完成标准</span><textarea required maxLength={500} value={acceptanceCriteria} onChange={(event)=>setAcceptance(event.target.value)} /></label><label><span>执行频率</span><select value={executionFrequency} onChange={(event)=>setExecutionFrequency(event.target.value as JourneyTask["execution_frequency"])}><option value="once">单次任务</option><option value="daily">每日任务</option><option value="weekly">每周任务</option><option value="monthly">每月任务</option></select></label>{executionFrequency==="weekly"&&<label><span>每周执行日（可选）</span><select value={preferredWeekday} onChange={(event)=>setPreferredWeekday(event.target.value)}><option value="">本周择时</option>{["周一","周二","周三","周四","周五","周六","周日"].map((day)=><option key={day}>{day}</option>)}</select></label>}{executionFrequency==="monthly"&&<label><span>每月执行日（可选）</span><input type="number" min="1" max="31" value={preferredMonthDay||""} onChange={(event)=>setPreferredMonthDay(event.target.value?Number(event.target.value):0)} /></label>}<div className="field-grid"><label><span>执行时间（可选）</span><input type="time" value={preferredTime} onChange={(event)=>setPreferredTime(event.target.value)} /></label><label><span>单次预计分钟</span><input type="number" min="15" max="1200" step="15" value={estimatedMinutes} onChange={(event)=>setMinutes(Number(event.target.value))} /></label></div><label><span>成果类型</span><select value={taskType} onChange={(event)=>setTaskType(event.target.value as JourneyTask["task_type"])}><option value="general">通用</option><option value="account_operation">账号运营</option><option value="reading">阅读</option><option value="finance">财务</option><option value="exercise">运动</option><option value="english">英语</option></select></label><div className="dialog-actions">{item&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除子任务"}</button>:<span/>}<button className="primary-button" disabled={busy||!mainTask.trim()||!title.trim()||!acceptanceCriteria.trim()}>{busy?"保存中…":"保存任务规则"}</button></div></form></div>;
}
...
function JourneyCreateDialog({ busy, onClose, onSave }: { busy: boolean; onClose: () => void; onSave: (values: { title: string; area: string; acceptanceCriteria: string; nextAction: string }) => void }) {
  ...
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog journey-dialog" onSubmit={(event)=>{event.preventDefault();onSave({title,area,acceptanceCriteria,nextAction});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">愿景闯关</span><h2>添加新关卡</h2><p>写清通关标准，路线可以在行动中调整。</p><div className="field-grid"><label><span>关卡名称</span><input required maxLength={80} value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="例如：建立稳定的英语表达能力" /></label><label><span>人生领域</span><select value={area} onChange={(event)=>setArea(event.target.value)}>{journeyAreas.map((item)=><option key={item}>{item}</option>)}</select></label></div><label><span>通关标准</span><textarea required value={acceptanceCriteria} onChange={(event)=>setAcceptance(event.target.value)} placeholder="怎样才算真正通关？" /></label><label><span>下一步行动</span><input required value={nextAction} onChange={(event)=>setNext(event.target.value)} placeholder="下一步最小行动" /></label><button className="primary-button full" disabled={busy}>{busy?"添加中…":"添加关卡"}</button></form></div>;
}
...
function TaskCompleteDialog({ action, busy, onClose, onSubmit }: { action: Action; busy: boolean; onClose: () => void; onSubmit: (values: Record<string, unknown>) => void }) {
  ...
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog complete-dialog" onSubmit={(event)=>{event.preventDefault();onSubmit({content,duration,feeling,category,amount,investmentPrincipal,investmentReturn,recordedAt,incomeType,sourceName,expenseScope});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className={`task-type ${action.task_type}`}>{taskTypeLabel(action.task_type)}任务</span><h2>提交完成成果</h2><p>{action.title}</p>{action.task_type==="exercise"?<><label><span>运动时间（分钟）</span><input type="number" min="1" max="600" required value={duration} onChange={(event)=>setDuration(Number(event.target.value))} /></label><label><span>运动感受</span><textarea required value={feeling} onChange={(event)=>setFeeling(event.target.value)} placeholder="身体状态、强度和恢复感受…" /></label></>:<label><span>{descriptionOnly ? "固定资产描述" : label}</span><textarea required={action.task_type==="reading"||action.task_type==="english"||action.task_type==="account_operation"||descriptionOnly} value={content} onChange={(event)=>setContent(event.target.value)} placeholder={descriptionOnly ? "例如：自住房一套，位于杭州，暂不估值" : placeholder} /></label>}{action.task_type==="finance"&&<><div className={descriptionOnly ? "field-grid single" : "field-grid"}><label><span>数据类型</span><select value={category} onChange={(event)=>setCategory(event.target.value as FinancialRecord["category"])}><option value="cash">现金</option><option value="reserve_fund">备用金</option><option value="fixed_asset">固定资产</option><option value="investment">投资</option><option value="income">收入</option><option value="fixed_expense">固定支出</option><option value="daily_expense">日常消费</option><option value="social_expense">请客</option><option value="exercise_expense">运动</option><option value="learning_expense">学习</option><option value="travel_expense">旅行</option></select></label>{!descriptionOnly&&category!=="investment"&&<label><span>金额（元）</span><input type="number" min="0" step="0.01" required value={amount} onChange={(event)=>setAmount(event.target.value)} /></label>}</div>{category==="investment"&&<div className="field-grid"><label><span>投资本金（元）</span><input type="number" min="0" step="0.01" required value={investmentPrincipal} onChange={(event)=>setInvestmentPrincipal(event.target.value)} /></label><label><span>本月收益（元，可为负）</span><input type="number" step="0.01" required value={investmentReturn} onChange={(event)=>setInvestmentReturn(event.target.value)} /></label></div>}{category==="income"&&<div className="field-grid"><label><span>收入类型</span><select value={incomeType} onChange={(event)=>setIncomeType(event.target.value as FinancialRecord["income_type"])}><option value="salary">工资收入</option><option value="non_salary">非工资收入</option></select></label><label><span>收入来源</span><input value={sourceName} onChange={(event)=>setSourceName(event.target.value)} placeholder="公司 / 客户 / 产品" /></label></div>}{/expense$/.test(category)&&<label><span>支出归属</span><select value={expenseScope} onChange={(event)=>setExpenseScope(event.target.value as FinancialRecord["expense_scope"])}><option value="personal">个人支出</option><option value="business">经营支出</option></select></label>}<label><span>记录日期</span><input type="date" value={recordedAt} onChange={(event)=>setDate(event.target.value)} /></label></>}<button className="primary-button full" disabled={busy}>{busy?"提交中…":"确认完成并保存成果"}</button></form></div>;
}

function FinanceDialog({ record, busy, onClose, onSave, onDelete }: { record?: FinancialRecord; busy: boolean; onClose: () => void; onSave: (values: Record<string,unknown>) => void; onDelete?: () => void }) {
  const initialCategory = record?.category === "property" ? "fixed_asset" : record?.category ?? "cash";
  const [category,setCategory]=useState<FinancialRecord["category"]>(initialCategory),[amount,setAmount]=useState(record && initialCategory !== "fixed_asset" ? String(record.amount) : ""),[investmentPrincipal,setInvestmentPrincipal]=useState(record?.category==="investment"?String(record.investment_principal||record.amount):""),[investmentReturn,setInvestmentReturn]=useState(record?.category==="investment"?String(record.investment_return||0):"0"),[note,setNote]=useState(record?.note ?? ""),[recordedAt,setDate]=useState(record?.recorded_at ?? new Date().toISOString().slice(0,10)),[incomeType,setIncomeType]=useState<FinancialRecord["income_type"]>(record?.income_type||"salary"),[sourceName,setSourceName]=useState(record?.source_name??""),[expenseScope,setExpenseScope]=useState<FinancialRecord["expense_scope"]>(record?.expense_scope||"personal"),[confirmDelete,setConfirmDelete]=useState(false);
  const descriptionOnly = category === "fixed_asset" || category === "property";
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog" onSubmit={(event)=>{event.preventDefault();onSave({category,amount,investmentPrincipal,investmentReturn,recordedAt,note,incomeType,sourceName,expenseScope});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">财务记录</span><h2>{record ? "编辑财务记录" : "添加一条数据"}</h2><p>现金与资产形成固定资产快照；投资、收入和支出进入所选日期对应的月账单。</p><div className={descriptionOnly ? "field-grid single" : "field-grid"}><label><span>类型</span><select value={category} onChange={(event)=>setCategory(event.target.value as FinancialRecord["category"])}><optgroup label="固定资产模块"><option value="cash">可用现金</option><option value="reserve_fund">备用金</option><option value="fixed_asset">固定资产</option></optgroup><optgroup label="月度记录模块"><option value="investment">投资</option><option value="income">收入</option><option value="fixed_expense">固定支出</option><option value="daily_expense">日常消费</option><option value="social_expense">请客</option><option value="exercise_expense">运动</option><option value="learning_expense">学习</option><option value="travel_expense">旅行</option></optgroup></select></label>{!descriptionOnly&&category!=="investment"&&<label><span>金额（元）</span><input required type="number" min="0" step="0.01" value={amount} onChange={(event)=>setAmount(event.target.value)} /></label>}</div>{category==="investment"&&<div className="field-grid"><label><span>投资本金（元）</span><input required type="number" min="0" step="0.01" value={investmentPrincipal} onChange={(event)=>setInvestmentPrincipal(event.target.value)} /></label><label><span>本月收益（元，可为负）</span><input required type="number" step="0.01" value={investmentReturn} onChange={(event)=>setInvestmentReturn(event.target.value)} /></label></div>}{category==="income"&&<div className="field-grid"><label><span>收入类型</span><select value={incomeType} onChange={(event)=>setIncomeType(event.target.value as FinancialRecord["income_type"])}><option value="salary">工资收入</option><option value="non_salary">非工资收入</option></select></label><label><span>收入来源</span><input value={sourceName} onChange={(event)=>setSourceName(event.target.value)} placeholder="公司 / 客户 / 产品" /></label></div>}{/expense$/.test(category)&&<label><span>支出归属</span><select value={expenseScope} onChange={(event)=>setExpenseScope(event.target.value as FinancialRecord["expense_scope"])}><option value="personal">个人支出</option><option value="business">经营支出</option></select></label>}<label><span>{descriptionOnly ? "固定资产描述" : "备注"}</span><textarea required={descriptionOnly} value={note} onChange={(event)=>setNote(event.target.value)} placeholder={descriptionOnly ? "例如：自住房一套，位于杭州，暂不估值" : "补充数据口径或说明（可选）"} /></label><label><span>日期</span><input type="date" value={recordedAt} onChange={(event)=>setDate(event.target.value)} /></label><div className="dialog-actions">{record&&onDelete?<button type="button" className={confirmDelete?"danger-button confirm":"danger-button"} disabled={busy} onClick={()=>confirmDelete?onDelete():setConfirmDelete(true)}>{confirmDelete?"再次点击确认删除":"删除记录"}</button>:<span/>}<button className="primary-button" disabled={busy}>{busy?"保存中…":"保存财务记录"}</button></div></form></div>;
}
