...
type FinancialRecord = { id: string; action_id: string | null; category: "cash" | "fixed_asset" | "investment" | "property" | "income" | "fixed_expense"; amount: number; note: string; recorded_at: string };
...
function Finance({ records, actions, busy, mutate }: { records: FinancialRecord[]; actions: Action[]; busy: boolean; mutate: (payload: Record<string, unknown>, success?: string) => Promise<boolean> }) {
  ...
  const cash = latest("cash"), fixedAsset = records.find((item) => item.category === "fixed_asset" || item.category === "property")?.amount ?? 0, investment = latest("investment"), income = flow("income"), expense = flow("fixed_expense");
  const currency = (value: number) => `¥${value.toLocaleString("zh-CN", { maximumFractionDigits: 2 })}`;
  return <><PageHeader kicker="现金流与资产底盘" title="当前财务情况"><button className="primary-button" onClick={() => setAdding(true)}>＋ 添加财务记录</button></PageHeader><div className="finance-grid"><article><span>可用现金</span><b>{currency(cash)}</b><small>最新快照</small></article><article><span>固定资产</span><b>{currency(fixedAsset)}</b><small>最新快照</small></article><article><span>投资</span><b>{currency(investment)}</b><small>最新快照</small></article><article><span>本月收入</span><b>{currency(income)}</b><small>累计记录</small></article><article><span>固定支出</span><b>{currency(expense)}</b><small>本月累计</small></article><article className="net-worth"><span>当前资产</span><b>{currency(cash + fixedAsset + investment)}</b><small>现金 + 固定资产 + 投资</small></article><article><span>结余率</span><b>{income ? `${Math.round((income - expense) / income * 100)}%` : "—"}</b><small>收入减固定支出</small></article></div><section className="finance-list"><div className="section-heading"><div><span className="eyebrow">全部财务记录</span><h3>每笔数据都可追溯到任务</h3></div></div>{records.length ? records.map((item) => <article key={item.id}><span className={`finance-category ${item.category}`}>{financeLabel(item.category)}</span><div><b>{currency(item.amount)}</b><p>{item.note || "未填写备注"}{item.action_id ? ` · 关联：${actions.find((action) => action.id === item.action_id)?.title || "任务"}` : ""}</p></div><time>{new Date(item.recorded_at).toLocaleDateString("zh-CN")}</time></article>) : <div className="empty-list"><b>还没有财务数据</b><p>添加现金、固定资产、投资、收入或固定支出，形成第一张财务快照。</p></div>}</section>{adding && <FinanceDialog actions={actions} busy={busy} onClose={() => setAdding(false)} onSave={async (values) => { const ok = await mutate({ action: "financial-record", ...values }, "财务记录已保存"); if (ok) setAdding(false); }} />}</>;
}

function financeLabel(category: FinancialRecord["category"]) { return { cash: "现金", fixed_asset: "固定资产", investment: "投资", property: "固定资产", income: "收入", fixed_expense: "固定支出" }[category]; }
...
function FinanceDialog({ actions, busy, onClose, onSave }: { actions: Action[]; busy: boolean; onClose: () => void; onSave: (values: Record<string,unknown>) => void }) {
  ...
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions
  return <div className="dialog-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><form className="dialog" onSubmit={(event)=>{event.preventDefault();onSave({category,amount,recordedAt,note,actionId});}}><button type="button" className="dialog-close" onClick={onClose}>×</button><span className="eyebrow">财务记录</span><h2>添加一条数据</h2><p>现金、固定资产与投资按最新快照展示，收入与固定支出按月累计。</p><div className="field-grid"><label><span>类型</span><select value={category} onChange={(event)=>setCategory(event.target.value as FinancialRecord["category"])}><option value="cash">现金</option><option value="fixed_asset">固定资产</option><option value="investment">投资</option><option value="income">收入</option><option value="fixed_expense">固定支出</option></select></label><label><span>金额（元）</span><input required type="number" min="0" step="0.01" value={amount} onChange={(event)=>setAmount(event.target.value)} /></label></div><label><span>关联任务（可选）</span><select value={actionId} onChange={(event)=>setActionId(event.target.value)}><option value="">不关联任务</option>{actions.map((item)=><option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label><span>日期</span><input type="date" value={recordedAt} onChange={(event)=>setDate(event.target.value)} /></label><label><span>备注</span><textarea value={note} onChange={(event)=>setNote(event.target.value)} /></label><button className="primary-button full" disabled={busy}>{busy?"保存中…":"保存财务记录"}</button></form></div>;
}
