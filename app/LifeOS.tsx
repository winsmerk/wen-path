  ...
  return <>
    ...
    <section className="journey-list">
      {visible.map((item) => <article key={item.id} className="journey-row">
        ...
        <div className="journey-copy"><div><span className={`area-pill ${areaTone[item.area] ?? "stone"}`}>{item.area}</span><span className={`status-text ${item.status}`}>{statusLabel(item.status)}</span>{item.evidence_review_status==="passed"&&<span className="evidence-pass">智能验收 {item.evidence_score}分</span>}</div><h3>{item.title}</h3><p>{item.acceptance_criteria}</p>{item.status === "active" && <small>下一步：{item.next_action}</small>}{item.evidence_review_status==="needs_more"&&<small className="evidence-needs-more">验收待补充（{item.evidence_score}分）：{item.evidence_review_feedback}</small>}{item.status === "completed" && item.evidence && <small>完成证据：{item.evidence}</small>}</div>
