...
async function settleFinancialMonths(db:D1Database,userId:string,currentPeriod:string,now:string) {
  ...
  await db.prepare(`INSERT INTO financial_monthly_bills (id,user_id,period,income_total,salary_income,non_salary_income,expense_total,business_expense,investment_principal,investment_return,business_profit,net_cash_flow,settled_at)
    SELECT ?||':'||substr(recorded_at,1,7),?,substr(recorded_at,1,7),
      ...
      SUM(CASE WHEN category IN ('fixed_expense','daily_expense','social_expense','exercise_expense','learning_expense','travel_expense') THEN amount ELSE 0 END),SUM(CASE WHEN category IN ('fixed_expense','daily_expense','social_expense','exercise_expense','learning_expense','travel_expense') AND expense_scope='business' THEN amount ELSE 0 END),
      ...
      SUM(CASE WHEN category='income' AND income_type='non_salary' THEN amount ELSE 0 END)-SUM(CASE WHEN category IN ('fixed_expense','daily_expense','social_expense','exercise_expense','learning_expense','travel_expense') AND expense_scope='business' THEN amount ELSE 0 END),
      SUM(CASE WHEN category='income' THEN amount ELSE 0 END)+SUM(CASE WHEN category='investment' THEN investment_return ELSE 0 END)-SUM(CASE WHEN category IN ('fixed_expense','daily_expense','social_expense','exercise_expense','learning_expense','travel_expense') THEN amount ELSE 0 END),?
...
export async function POST(request: Request) {
  ...
  if (body.action === "initialize") {
    ...
    if (task.task_type === "finance") {
      ...
      if (!["cash", "reserve_fund", "fixed_asset", "investment", "property", "income", "fixed_expense", "daily_expense", "social_expense", "exercise_expense", "learning_expense", "travel_expense"].includes(category) || !Number.isFinite(amount) || amount < 0 || !Number.isFinite(investmentPrincipal)||investmentPrincipal<0||!Number.isFinite(investmentReturn) || (descriptionOnly && !content)) return NextResponse.json({ error: "finance_required" }, { status: 400 });
    ...
    if (!["cash","reserve_fund","fixed_asset","investment","property","income","fixed_expense","daily_expense","social_expense","exercise_expense","learning_expense","travel_expense"].includes(category) || !Number.isFinite(amount) || amount < 0 || !Number.isFinite(investmentPrincipal)||investmentPrincipal<0||!Number.isFinite(investmentReturn) || (descriptionOnly && !note)) return NextResponse.json({error:"invalid_finance"},{status:400});
