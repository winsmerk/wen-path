...
export async function GET() {
  ...
  const [profile, journeys, outcomes, actions, checkins, reviews] = await Promise.all([
    ...
    db.prepare("SELECT * FROM journeys WHERE user_id = ? ORDER BY sequence_number").bind(identity.userId).all(),
...
export async function POST(request: Request) {
  ...
  if (body.action === "initialize") {
    ...
    if (body.status === "active") {
      const count = await db.prepare("SELECT COUNT(*) AS total FROM journeys WHERE user_id = ? AND status = 'active'")
        .bind(identity.userId).first<{ total: number }>();
      if ((count?.total ?? 0) >= 5) return NextResponse.json({ error: "active_limit" }, { status: 409 });
    }
    await db.prepare("UPDATE journeys SET status = ? WHERE id = ? AND user_id = ?")
