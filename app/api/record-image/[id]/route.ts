...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const image = await db.prepare("SELECT object_key,content_type FROM record_images WHERE id=? AND user_id=?").bind(id, identity.userId).first<{ object_key: string; content_type: string }>();
