...
export async function POST(request: Request) {
  ...
  const footprint = await db.prepare("SELECT id,name FROM footprints WHERE id=? AND user_id=? AND geometry_version < 3")
  ...
  await db.prepare("UPDATE footprints SET latitude=?,longitude=?,geometry_json=?,geometry_version=3,updated_at=? WHERE id=? AND user_id=?")
