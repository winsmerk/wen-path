...
export async function POST(request:Request){
  ...
  if(action==="save-stage"){
    ...
    const values=[goalId,title,description,typeKey,mode,frequency,occurrences,JSON.stringify(weekdays),JSON.stringify(monthDays),JSON.stringify(times),scheduledDate||null,startDate||null,endDate||null,minutes,priority,now];
    if(id){await db.prepare("UPDATE task_definitions_v2 SET goal_id=?,title=?,description=?,type_key=?,mode=?,frequency=?,occurrences=?,weekdays_json=?,month_days_json=?,times_json=?,scheduled_date=?,start_date=?,end_date=?,estimated_minutes=?,priority=?,updated_at=? WHERE id=? AND user_id=?").bind(...values,id,userId).run();await db.prepare("DELETE FROM task_instances_v2 WHERE user_id=? AND definition_id=? AND status!='completed' AND user_adjusted=0").bind(userId,id).run();}
    else await db.prepare("INSERT INTO task_definitions_v2 (id,user_id,goal_id,title,description,type_key,mode,frequency,occurrences,weekdays_json,month_days_json,times_json,scheduled_date,start_date,end_date,estimated_minutes,priority,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?)").bind(crypto.randomUUID(),userId,...values.slice(0,-1),now,now).run();
    ...
    const period=clean(body.period,7),title=clean(body.title,160),goalIds=[...new Set(list(body.goalIds,100).map((item)=>clean(item,100)).filter(Boolean))];if(!/^\d{4}-\d{2}$/.test(period)||!goalIds.length)return NextResponse.json({error:"invalid_month_plan"},{status:400});
    ...
    const current=await db.prepare("SELECT status FROM task_instances_v2 WHERE id=? AND user_id=?").bind(id,userId).first<{status:string}>();if(!current)return NextResponse.json({error:"not_found"},{status:404});
