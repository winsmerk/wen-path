...
import { dispatchDueMemos, sendServerChanMessage } from "@/lib/memos";
import { ensureSchema, getD1, getServerChanSendKey, getWorkspaceIdentity, seedWorkspace } from "@/lib/workspace";
...
export async function GET() {
  ...
  await dispatchDueMemos(context.db, sendKey, context.identity.userId);
  ...
  return NextResponse.json({ memos: rows.results, wechatConfigured: Boolean(sendKey) });
}
...
export async function POST(request: Request) {
  ...
  if (action === "save") {
    const id = clean(body.id, 100), title = clean(body.title, 120), content = clean(body.content, 4000), remindAtInput = clean(body.remindAt, 40), wechatEnabled = body.wechatEnabled !== false ? 1 : 0;
    ...
    const remindAt = parsed.toISOString(), deliveryStatus = wechatEnabled ? "pending" : "not_requested";
    if (id) {
      const result = await context.db.prepare("UPDATE memos SET title=?,content=?,remind_at=?,wechat_enabled=?,delivery_status=?,attempt_count=0,sent_at=NULL,last_error='',updated_at=? WHERE id=? AND user_id=?").bind(title, content, remindAt, wechatEnabled, deliveryStatus, now, id, context.identity.userId).run();
      if (!result.meta.changes) return NextResponse.json({ error: "not_found" }, { status: 404 });
    } else {
      await context.db.prepare("INSERT INTO memos (id,user_id,title,content,remind_at,status,wechat_enabled,delivery_status,attempt_count,sent_at,last_error,created_at,updated_at) VALUES (?,?,?,?,?,'pending',?,?,0,NULL,'',?,?)").bind(crypto.randomUUID(), context.identity.userId, title, content, remindAt, wechatEnabled, deliveryStatus, now, now).run();
    }
  ...
  } else if (action === "test-wechat") {
    const sendKey = getServerChanSendKey();
    if (!sendKey) return NextResponse.json({ error: "wechat_not_configured" }, { status: 503 });
    try {
      await sendServerChanMessage(sendKey, "wen flow 微信提醒测试", "连接成功。之后备忘录到达设定时间时，会自动发送到这里。");
    } catch {
      return NextResponse.json({ error: "wechat_send_failed" }, { status: 502 });
    }
  } else {
